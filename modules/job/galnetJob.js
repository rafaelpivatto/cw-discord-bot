...
exports.execute = (client) => {
    ...
    //Execute every hour
    schedule.scheduleJob(utils.getGlobalConfig('job.galnet.cron'), () => {
        
        client.guilds.array().forEach(guild => {
            if (utils.isGuildConfigEnabled(guild, 'galnet.enabled')) {

                logger.info(`${logName} started for guild: ${guild.name}`);

                const channel = guild.channels.find('name', utils.getGuildConfig(guild, 'galnet.channel'));
                
                if (channel) {
                    const query = {_id: `${guild.id}--${guild.name}`};
                    mongoConnection.find(logName, query, 'galnet', (error, results) => {
                        if (error) {
                            logger.error(`${logName} Error on retrieving informations`, {'query': query, 'error': error});
                        }
    
                        if (results.length == 0 || results.length > 0) {
                            const fullUrl = utils.getGlobalConfig('job.galnet.url') + getDate();
                            getFromUrl.getHtml(logName, fullUrl, (error, body) => {
                                
                                if (error) return;
    
                                let dataToSave = {
                                    _id: `${guild.id}--${guild.name}`,
                                    updateDate: new Date(),
                                    articles: []
                                };
                                ...
                                for(let i = 0; i < articleTitles.length; i++) {
                                    ...
                                    let embed = new RichEmbed()
                                        .setColor(utils.getGuildConfig(guild, 'galnet.color'))
                                        .setTimestamp()
                                        .setTitle('**' + title + '**')
                                        .setDescription(desc)
                                        .setURL(fullUrl)
                                        .setThumbnail(utils.getGuildConfig(guild, 'galnet.thumb'));
                                    ...
                                    mongoConnection.saveOrUpdate(logName, dataToSave, 'galnet', () => {});
                                }
                            });
                        }
                    });
